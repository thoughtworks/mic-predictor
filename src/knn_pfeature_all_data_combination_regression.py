...
def get_tuned_regressor_train_test_scores(X, y, data_name, random_state=42):
    ...
    grid_search = get_tuned_model(knn, param_grid, X_train, y_train, n_jobs=2)
    best_grid = grid_search.best_estimator_
    best_grid.fit(X_train, y_train)
    y_pred = best_grid.predict(X_test)
    return {'data':data_name, 'Best_params':grid_search.best_params_, 'scores':get_score(y_test, y_pred)}
...
def main():  
    ...
    ray_regressor = ray.remote(get_tuned_regressor_train_test_scores)
    ray.init(num_cpus=6)
    futures = [ray_regressor.remote(data,df['MIC'],name) for name, data in data_dict.items()]
